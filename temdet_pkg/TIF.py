...
#    Updated: 2020/04/21 15:49:13 by winshare         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
class TIF():
    ...
    def readtif(self,filename):
        ...
        assert self.dataset != None,"Can't Read Dataset ,Invalid tif file : "+filename
    ...
    def fast_percentager_strentching(self,image=None,percentage=2,sample=10000):
        """
        Image ndarray:(W,H,C)
        Percentage N(0-100)%
        """
    ...
    def set(self,Data):
        ...
        """
        if len(Data.shape)==2:
            assert type(Data[0,0])!=bool,'Polygonize Data ( SetRasterData(Data) ) Must be bool Ndarray,But now in '+str(type(Data[0,0]))
        self.RasterSet=True
    ...
    def resize_raster(self,resize_ratio=0.5):
        ...
        self.resizedimage=cv2.resize(self.image_nparray,size,interpolation=cv2.INTER_AREA)
